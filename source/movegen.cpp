...
{
    if ((attacker_side == white) && (pawn_attacks[black][square] & bitboards[P])) return 1;
    if ((attacker_side == black) && (pawn_attacks[white][square] & bitboards[p])) return 1;
    if (knight_attacks[square] & ((attacker_side == white) ? bitboards[N] : bitboards[n])) return 1;
    if (get_bishop_attacks(square, occupancies[both]) & ((attacker_side == white) ? bitboards[B] : bitboards[b])) return 1;
    if (get_rook_attacks(square, occupancies[both]) & ((attacker_side == white) ? bitboards[R] : bitboards[r])) return 1;
    if (get_queen_attacks(square, occupancies[both]) & ((attacker_side == white) ? bitboards[Q] : bitboards[q])) return 1;
    if (king_attacks[square] & ((attacker_side == white) ? bitboards[K] : bitboards[k])) return 1;
    return 0;
}
