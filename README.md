...
* **Parallel Search:** **Lazy SMP** — independent threads sharing the transposition table, with per-thread depth skipping for diversity (the legacy ABDADA scheme remains available via the `LazySMP` toggle).
